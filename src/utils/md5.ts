/**
 * Compact MD5 implementation (UTF-8 text / ArrayBuffer).
 * For local tooling only; prefer SHA-256 for security-sensitive use.
 */
...
function md5blk(bytes: Uint8Array): number[] {
  ...
  const tail = n % 64
  const padding = tail < 56 ? 56 - tail : 120 - tail
  const total = n + padding + 8
